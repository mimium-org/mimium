...
define void @closurefun(double %time0, %capture0* %clsarg_closurefun) {
entry:
  %0 = load %capture0, %capture0* %clsarg_closurefun
  %ptr_fv_globalvar = extractvalue %capture0 %0, 0
  %fv_globalvar = load double, double* %ptr_fv_globalvar
  call void @printlndouble(double 1.000000e+02)
  call void @printlndouble(double %time0)
  call void @printlndouble(double %fv_globalvar)
  ret void
}
