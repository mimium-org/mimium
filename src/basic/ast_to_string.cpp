...
ExprStringVisitor::ExprStringVisitor(std::ostream& output, StatementStringVisitor& parent,
                                     Mode mode)
...
void ExprStringVisitor::operator()(const ast::Number& ast) { output << ast.value; }
void ExprStringVisitor::operator()(const ast::String& ast) { output << ast.value; }
void ExprStringVisitor::operator()(const ast::Op& ast) {
  output << format.lpar << ast::op_str.at(ast.op);
  if (ast.lhs.has_value()) { output << format.delim << *ast.lhs.value(); }
  output << format.delim << *ast.rhs << format.rpar;
}
void ExprStringVisitor::operator()(const ast::Rvar& ast) { output << ast.value; }
void ExprStringVisitor::operator()(const ast::Self& /*ast*/) { output << "self"; }
...
void ExprStringVisitor::operator()(const ast::StructAccess& ast) {
  output << format.lpar << "structaccess" << format.delim << format.lpar_a << *ast.stru
         << format.delim << *ast.field << format.rpar_a << format.rpar;
}
...
void ExprStringVisitor::operator()(const ast::ArrayAccess& ast) {
  output << format.lpar << "arrayaccess" << format.delim << *ast.array << format.delim << ast.index
         << format.rpar;
}
...
void StatementStringVisitor::operator()(const ast::Assign& ast) {
  output << format.lpar << "assign" << format.delim << ast.lvar << format.delim << *ast.expr
         << format.rpar;
}
...
void StatementStringVisitor::operator()(const ast::If& ast) {
  output << format.lpar << "if" << format.delim << *ast.cond << format.delim << ast.then_stmts;
  if (ast.else_stmts.has_value()) { output << format.delim << ast.else_stmts.value(); }
  output << format.rpar;
}
